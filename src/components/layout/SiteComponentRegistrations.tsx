...
export function registerSiteComponents() {
  const logger = useLogger('SiteComponentRegistrations', { category: LogCategory.SYSTEM });
  
  try {
    logger.info('Registering site components');
    ...
    logger.info('Site components registered successfully', {
      details: { componentsCount: componentRegistry.getRegisteredComponents().length }
    });
  } catch (error) {
    logger.error('Error registering site components', {
