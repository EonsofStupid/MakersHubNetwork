...
export function DynamicKeyframes() {
  ...
  useEffect(() => {
    ...
    if (!rgbPrimary) {
      logger.warn('Failed to convert primary color to RGB, using fallback', { details: { color: primary } });
      rgbPrimary = { r: 0, g: 240, b: 255 }; // Fallback cyan
    }
    
    let rgbSecondary = hexToRgb(secondary);
    if (!rgbSecondary) {
      logger.warn('Failed to convert secondary color to RGB, using fallback', { details: { color: secondary } });
      rgbSecondary = { r: 255, g: 45, b: 110 }; // Fallback pink
    }
