...
export function ThemeList() {
  ...
  useEffect(() => {
    const fetchThemes = async () => {
      try {
        ...
        logger.info('Themes fetched successfully', { details: { count: data.length } });
      } catch (err) {
  ...
  const createTheme = async () => {
    ...
    try {
      ...
      // Use defaults from an existing theme
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          design_tokens,
          component_tokens: defaultTheme?.component_tokens || [],
          created_by: user.id,
          status: 'draft'
        })
        .select()
      ...
      setThemes(prevThemes => [data as Theme, ...prevThemes]);
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          design_tokens: sourceTheme.design_tokens,
          component_tokens: sourceTheme.component_tokens,
          created_by: user.id,
          status: 'draft',
  ...
  const publishTheme = async (themeId: string) => {
    try {
      ...
      setThemes(prevThemes => prevThemes.map(theme => 
        theme.id === themeId ? { ...theme, status: 'published' } : theme
      ));
